...
class RetLayer:
    ...
    def __init__(self, K, f, T_what, T_pixel = 1.2, COSINE = True, RNG = 42, VERBOSE = 1, MAX_ITER = 300, N_INIT = 10):
        self.K = K
        self.f = f
        self.p = f // 2
        self.RNG = RNG
    ...
    def fit(self,x):
        ...
        self.km = KMeans(n_clusters=K,init='k-means++',
            n_init=self.N_INIT,max_iter=self.MAX_ITER,
            verbose=self.VERBOSE,random_state=self.RNG)
        self.km.fit(v)
    ...
    def decode(self,x):
        ...
        n,n_H,n_W,K = x.shape
        x_win = np.squeeze(view_as_windows( np.pad(x,((0,0),(p,p),(p,p),(0,0))) ,(1,f,f,K),(1,1,1,K)))
...
class sparseWWencoder:
    def __init__(self, K, f, T_what, Q, T_pixel = 1.2, COSINE = True, RNG = 42, VERBOSE = 1, MAX_ITER = 300, N_INIT = 10):
        self.K = K
        self.f = f
        self.T_what = T_what
        self.Q = Q
        ...
        self.MAX_ITER = MAX_ITER
        self.N_INIT = N_INIT
        self.retlayer = RetLayer(K,f,T_what,T_pixel=T_pixel,
                                 COSINE=COSINE,RNG=RNG,VERBOSE=VERBOSE,
...
class AttentionMatcher:
    def __init__(self, K, f, T_what, T_where, Q,
                 T_pixel = 1.2, COSINE = True, RNG = 42, VERBOSE = 1,
                 MAX_ITER = 300, N_INIT = 10):
        self.K = K
        self.T_where = T_where
        self.encoder = sparseWWencoder(K, f, T_what, Q,
                                       T_pixel = T_pixel, COSINE = COSINE,
    ...
    def fit(self,x_train,y_train):
        ...
        K = self.K
        pw = T_where//2
        x_win = np.squeeze(view_as_windows( np.pad( x_enc, ((0,0),(pw,pw),(pw,pw),(0,0)) ) , (1,T_where,T_where,K)))
