...
from plots import plot_multiple_line_charts
...
if small_test:
    trn_imgs, trn_lbls, test_imgs, test_lbls = read_mnist(
        dim=[28, 28], n_train=4000, one_hot=False
    )
else:
    trn_imgs, trn_lbls, test_imgs, test_lbls = read_mnist(
        dim=[28, 28], n_train=60000, one_hot=False
    )

list_Fs = [1, 2]  # size of features, Fs = 1 results in a 3by3 filter size (2Fs+1)
list_Tw = [0.8, 0.85, 0.9]  # Treshod for keeping or discarding a detected feature
list_fos = [1, 3, 5, 6, 7]  # stored patterns

f = open("whatwhere/results/multiple_runs.csv", "w")
f.write(
