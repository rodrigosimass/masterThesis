...
def load_or_compute_will(run_name, X_trn, factor_of_stored):
    ...
    try:
        will = pickle.load(open(f"whatwhere/pickles/{run_name}__will.p", "rb"))
        print(f"loaded will from pickle: pickles/{run_name}__will.p")
    except (OSError, IOError) as _:
        will = train(X_trn, M)
        sps_will = csr_matrix(will)
        pickle.dump(sps_will, open(f"whatwhere/pickles/{run_name}__will.p", "wb"))
        print(f"saving trained willshaw to pickles/{run_name}_will.p")
    return will